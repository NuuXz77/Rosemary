                        ...
                        <div class="alert alert-soft alert-info">
                            ...
                            <div class="text-sm">
                                ...
                                <ul class="list-disc list-inside space-y-1">
                                    <li><strong>Kolom A (Nama Material):</strong> nama material/bahan (wajib)</li>
                                    <li><strong>Kolom B (Kategori):</strong> nama kategori material persis seperti data master (wajib)</li>
                                    <li><strong>Kolom C (Satuan):</strong> nama satuan persis seperti data master (wajib)</li>
                                    <li><strong>Kolom D (Supplier):</strong> nama supplier (opsional, boleh kosong)</li>
                                    <li><strong>Kolom E (Harga):</strong> angka harga modal per satuan (wajib)</li>
                                    <li><strong>Kolom F (Minimum Stok):</strong> angka stok minimum (wajib)</li>
                                    <li><strong>Kolom G (Status):</strong> active atau inactive (opsional, default: active)</li>
                                </ul>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col items-center justify-center p-6 border-2 border-dashed border-base-300 rounded-lg bg-base-200/50 hover:bg-base-200 transition-colors h-full min-h-[200px]">
                            ...
                            @else
                                ...
                                <label for="file-upload-material" class="btn btn-primary btn-sm gap-2 cursor-pointer">
                                    <x-heroicon-o-folder-open class="w-4 h-4" />
                                    Pilih File
                                </label>
                                <input id="file-upload-material" type="file" wire:model="file" accept=".xlsx,.xls" class="hidden" />
                            @endif
                ...
                <div class="flex flex-col sm:flex-row gap-3 mb-4 p-3 bg-base-200 rounded-lg">
                    ...
                    @if (count($availableCategories) > 0)
                        <div class="min-w-[180px]">
                            ...
                            <select wire:model.live="filterCategory" class="select select-sm select-bordered w-full bg-base-100">
                                <option value="all">Semua Kategori</option>
                                @foreach ($availableCategories as $category)
                                    @php
                                        $categoryCount = collect($previewData)->where('category_name', $category)->count();
                                    @endphp
                                    <option value="{{ $category }}">{{ $category }} ({{ $categoryCount }})</option>
                                @endforeach
                            </select>
                        </div>
                    @endif

                    @if ($filterSheet !== 'all' || $filterCategory !== 'all')
                        <div class="flex items-end">
                            <button wire:click="$set('filterSheet', 'all'); $set('filterCategory', 'all')" class="btn btn-xs btn-ghost text-error gap-1">
                ...
                <table class="table table-zebra table-sm">
                    <thead>
                        <tr>
                            ...
                            <th class="w-36">Supplier</th>
                            <th class="w-24 text-right">Harga</th>
                            <th class="w-24 text-right">Min. Stok</th>
                    ...
                    <tbody>
                            ...
                            @foreach ($filteredPreviewData as $index => $row)
                                ...
                                <tr class="hover:bg-base-200 {{ isset($row['has_error']) && $row['has_error'] ? 'bg-error/10' : '' }}">
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.category_name"
                                            class="input input-xs input-bordered w-full"
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.unit_name"
                                            class="input input-xs input-bordered w-full"
                                            placeholder="Satuan"
                                        >
                                    </td>
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.supplier_name"
                                            class="input input-xs input-bordered w-full"
                                    ...
                                    <td>
                                        <input
                                            ...
                                            step="0.01"
                                            min="0"
                                            wire:model.live="previewData.{{ $rowKey }}.price"
                                    ...
                                    <td>
                                        <input
                                            ...
                                            step="0.01"
                                            min="0"
                                            wire:model.live="previewData.{{ $rowKey }}.minimum_stock"
                                            class="input input-xs input-bordered w-full text-right"
                                            placeholder="0"
                                        >
                                    </td>
                        ...
                        @elseif (is_array($previewData) && count($previewData) > 0)
                            <tr>
                                <td colspan="{{ count($availableSheets) > 1 ? 11 : 10 }}" class="text-center py-10">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        <x-heroicon-o-funnel class="w-10 h-10 opacity-30" />
                                        <p class="font-semibold">Tidak ada data untuk filter ini</p>
                                        <button wire:click="$set('filterSheet', 'all'); $set('filterCategory', 'all')" class="btn btn-xs btn-ghost text-primary">Reset Filter</button>
                                    </div>
                                </td>
                            </tr>
                        @else
                            <tr>
                                <td colspan="10" class="text-center py-12">
                                    <div class="flex flex-col items-center gap-3 text-gray-400">
                                        ...
                                        <p class="text-sm">Upload file Excel untuk melihat data material</p>
                                    </div>
                                </td>
                            </tr>
                        @endif
