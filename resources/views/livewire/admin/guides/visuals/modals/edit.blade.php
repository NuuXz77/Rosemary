<div>
    <x-form.modal
        modalId="edit-guide-visual-modal"
    ...
    >
        ...
        @if($mediaFile)
            <div class="mt-3 rounded-xl border border-base-300 p-3 bg-base-200/40 max-w-sm">
                <p class="text-xs font-semibold mb-2">Preview Upload Baru</p>
                <img src="{{ $mediaFile->temporaryUrl() }}" alt="Preview media" class="rounded-lg w-full h-40 object-cover" />
            </div>
        @elseif($media_url)
            <div class="mt-3 rounded-xl border border-base-300 p-3 bg-base-200/40 max-w-sm">
                <p class="text-xs font-semibold mb-2">Media Saat Ini</p>
                <img src="{{ $media_url }}" alt="Current media" class="rounded-lg w-full h-40 object-cover" />
            </div>
        @endif

        <div class="alert alert-info mt-3 text-xs">
            <x-heroicon-o-information-circle class="w-4 h-4" />
            <span>Isi URL manual atau upload file baru. Upload akan mengganti media lama jika file lama tersimpan lokal.</span>
        </div>
