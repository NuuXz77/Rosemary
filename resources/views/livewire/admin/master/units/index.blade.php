<div>
        ...
        <div class="card-body p-6">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div class="flex items-center gap-2">
                    <x-heroicon-o-scale class="w-6 h-6 text-primary" />
                    <h2 class="text-xl font-bold">Daftar Satuan (Unit)</h2>
                </div>
                <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                    <div class="join w-full md:w-64">
                        <label class="input input-sm input-bordered join-item flex items-center gap-2 w-full">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="search" class="grow" placeholder="Cari satuan..." />
                        </label>
                    </div>
            ...
            <x-partials.table :columns="[
            ...
            ]" :data="$units" emptyMessage="Belum ada data satuan.">
                @foreach ($units as $index => $unit)
                    <tr wire:key="unit-{{ $unit->id }}" class="hover:bg-base-200/50 transition-colors">
                        ...
                        <td>
                            <div class="font-bold uppercase">{{ $unit->name }}</div>
                            <div class="text-xs text-base-content/40 italic">Dibuat {{ $unit->created_at->diffForHumans() }}</div>
                        </td>
                        <td>
                            <div @class([
                                'badge badge-sm gap-1.5',
                                'badge-success text-white' => $unit->status,
                                'badge-ghost' => !$unit->status,
                            ])>
                                <div @class([
                                    'w-1.5 h-1.5 rounded-full',
                                    'bg-white' => $unit->status,
                                    'bg-base-content/30' => !$unit->status,
                                ])></div>
    ...
    <!-- Unit Modal -->
    <x-partials.modal id="unit-modal" :title="$isEdit ? 'Edit Satuan' : 'Tambah Satuan Baru'">
        <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}" class="space-y-4">
            <div class="form-control">
                <label class="label"><span class="label-text font-semibold">Nama Satuan</span></label>
                <input type="text" wire:model="name" class="input input-bordered w-full @error('name') input-error @enderror" placeholder="Contoh: kg, pcs, liter, box, buah..." />
            ...
            <div class="form-control">
                ...
                <span class="text-xs text-base-content/50">Satuan nonaktif tidak akan muncul di pilihan form input lainnya.</span>
            </div>

            <div class="modal-action">
                <button type="button" class="btn" onclick="document.getElementById('unit-modal').close()">Batal</button>
    ...
    <x-partials.modal id="delete-modal" title="Konfirmasi Hapus">
        <div class="flex flex-col items-center text-center py-4">
            ...
            <p class="text-base-content/60 mt-1">Data satuan yang dihapus tidak dapat dikembalikan. Pastikan satuan tidak lagi digunakan dalam material.</p>
        </div>
