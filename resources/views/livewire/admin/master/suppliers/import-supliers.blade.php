                        ...
                        <div class="alert alert-soft alert-info">
                            ...
                            <div class="text-sm">
                                ...
                                <ul class="list-disc list-inside space-y-1">
                                    <li><strong>Kolom A (Nama Supplier):</strong> nama supplier (wajib)</li>
                                    <li><strong>Kolom B (Telepon):</strong> nomor telepon (opsional)</li>
                                    <li><strong>Kolom C (Deskripsi):</strong> catatan/description supplier (opsional)</li>
                                    <li><strong>Kolom D (Frekuensi):</strong> sering, sedang, atau jarang (opsional, default: sedang)</li>
                                </ul>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col items-center justify-center p-6 border-2 border-dashed border-base-300 rounded-lg bg-base-200/50 hover:bg-base-200 transition-colors h-full min-h-[200px]">
                            ...
                            @else
                                ...
                                <label for="file-upload-suppliers" class="btn btn-primary btn-sm gap-2 cursor-pointer">
                                    <x-heroicon-o-folder-open class="w-4 h-4" />
                                    Pilih File
                                </label>
                                <input id="file-upload-suppliers" type="file" wire:model="file" accept=".xlsx,.xls" class="hidden" />
                            @endif
            ...
            @if (is_array($previewData) && count($previewData) > 0)
                <div class="flex flex-col sm:flex-row gap-3 mb-4 p-3 bg-base-200 rounded-lg">
                    ...
                    <div class="min-w-[180px]">
                        ...
                        <select wire:model.live="filterStatus" class="select select-sm select-bordered w-full bg-base-100">
                    ...
                    @if ($filterSheet !== 'all' || $filterStatus !== 'all')
                        <div class="flex items-end">
                            <button wire:click="$set('filterSheet', 'all'); $set('filterStatus', 'all')" class="btn btn-xs btn-ghost text-error gap-1">
                ...
                <table class="table table-zebra table-sm">
                    <thead>
                        <tr>
                            ...
                            @endif
                            <th>Nama Supplier</th>
                            <th class="w-36">Telepon</th>
                            <th>Deskripsi</th>
                            <th class="w-24 text-center">Frekuensi</th>
                        ...
                        @if (count($filteredPreviewData) > 0)
                            @foreach ($filteredPreviewData as $index => $row)
                                ...
                                <tr class="hover:bg-base-200 {{ isset($row['has_error']) && $row['has_error'] ? 'bg-error/10' : '' }}">
                                    ...
                                    @if (count($availableSheets) > 1)
                                        <td><span class="badge badge-ghost badge-xs">{{ $row['sheet_name'] ?? '-' }}</span></td>
                                    @endif
                                    <td>
                                        <input
                                            type="text"
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.phone"
                                            class="input input-xs input-bordered w-full"
                                            placeholder="Telepon"
                                        >
                                    </td>
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.description"
                                            class="input input-xs input-bordered w-full"
                            ...
                            <tr>
                                <td colspan="{{ count($availableSheets) > 1 ? 8 : 7 }}" class="text-center py-10">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        ...
                                        <button wire:click="$set('filterSheet', 'all'); $set('filterStatus', 'all')" class="btn btn-xs btn-ghost text-primary">Reset Filter</button>
                                    </div>
                                </td>
                            </tr>
                        @else
                            <tr>
                                <td colspan="7" class="text-center py-12">
                                    <div class="flex flex-col items-center gap-3 text-gray-400">
                                        ...
                                        <p class="text-sm">Upload file Excel untuk melihat data supplier</p>
                                    </div>
                                </td>
                            </tr>
                        @endif
...
<script>
    $wire.on('redirect-after-import', () => {
        setTimeout(() => {
            window.location.href = '{{ route('master.suppliers.index') }}';
        }, 1500);
    });
</script>
@endscript
