            ...
            <div class="card-body">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col items-center justify-center p-6 border-2 border-dashed border-base-300 rounded-lg bg-base-200/50 hover:bg-base-200 transition-colors h-full min-h-50">
        ...
        <div class="card-body">
            ...
            @if (is_array($previewData) && count($previewData) > 0)
                <div class="flex flex-col sm:flex-row gap-3 mb-4 p-3 bg-base-200 rounded-lg">
                    ...
                    @if (count($availableDivisions) > 0)
                        <div class="min-w-45">
                            <p class="text-xs text-gray-500 font-semibold mb-1 flex items-center gap-1">
                                <x-heroicon-o-building-office-2 class="w-3.5 h-3.5" />
                                Filter Divisi
                            </p>
                            <select wire:model.live="filterDivision" class="select select-sm select-bordered w-full bg-base-100">
                    ...
                    @if ($filterSheet !== 'all' || $filterCategory !== 'all' || $filterDivision !== 'all')
                        <div class="flex items-end">
                            <button wire:click="$set('filterSheet', 'all'); $set('filterCategory', 'all'); $set('filterDivision', 'all')" class="btn btn-xs btn-ghost text-error gap-1">
                ...
                <table class="table table-zebra table-sm">
                    ...
                    <tbody>
                            ...
                            @foreach ($filteredPreviewData as $index => $row)
                                ...
                                <tr class="hover:bg-base-200 {{ isset($row['has_error']) && $row['has_error'] ? 'bg-error/10' : '' }}">
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.barcode"
                                            class="input input-xs input-bordered w-full"
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            wire:model.live="previewData.{{ $rowKey }}.division_name"
                                            class="input input-xs input-bordered w-full"
                                            placeholder="Divisi"
                                        >
                                    </td>
                        ...
                        @elseif (is_array($previewData) && count($previewData) > 0)
                            <tr>
                                <td colspan="{{ count($availableSheets) > 1 ? 10 : 9 }}" class="text-center py-10">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        <x-heroicon-o-funnel class="w-10 h-10 opacity-30" />
                                        <p class="font-semibold">Tidak ada data untuk filter ini</p>
                                        <button wire:click="$set('filterSheet', 'all'); $set('filterCategory', 'all'); $set('filterDivision', 'all')" class="btn btn-xs btn-ghost text-primary">Reset Filter</button>
                                    </div>
                                </td>
                            </tr>
                        @else
